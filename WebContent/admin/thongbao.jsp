							...
							<table width="100%"
								...
								<tbody>
									...
									<tr class="odd gradeX">
										<td><%=tb.getTieuDe()%></td>
										<%
											if (tb.getNoiDung().length() > 110) {
										%>
										<td><%=tb.getNoiDung().substring(0, 105) + "..."%></td>
										<%
											} else {
										%>
										<td><%=tb.getNoiDung()%></td>
										...
										<td class="text-center"><a
											href="ThongBaoServlet?command=edit&id=<%=tb.getID()%>">Sửa</a></td>
										<td class="text-center"><a
											href="ThongBaoServlet?command=delete&id=<%=tb.getID()%>"
											onclick="return confirm('Bạn có chắc chắn xóa thông báo này không?')">Xóa</a></td>
									</tr>
									<%
										}
									%>
